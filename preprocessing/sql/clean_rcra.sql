...
drop table if exists cleaned.rcra_facilities;
create table cleaned.rcra_facilities as (
    select
    id_number,
...
drop table if exists cleaned.rcra_handler;
create table cleaned.rcra_handler as (
    select
    ...
    inner join cleaned.rcra_facilities as f on h.epa_handler_id = f.id_number
);

---- 3. create filtered cmecomp3 table with facility data
drop table if exists cleaned.rcra_cmecomp3;
create table cleaned.rcra_cmecomp3 as (
    ...
    from rcra.cmecomp3 c inner join cleaned.rcra_handler h on c.handler_id = h.epa_handler_id
...
drop table if exists cleaned.rcra_reporting;
create table cleaned.rcra_reporting as (
    ...
    inner join cleaned.rcra_facilities f
    on r.handler_id = f.id_number
);

---- 5. create entity id table
drop table if exists cleaned.entity_id;
create table if not exists cleaned.entity_id as(
    select distinct id_number
    from cleaned.rcra_facilities
    order by id_number
);
alter table cleaned.entity_id add column id int generated always as identity primary key;

---- 6. create event id table
drop table if exists cleaned.event_id;
create table if not exists cleaned.event_id as(
    select distinct epa_handler_id, receive_date
    from cleaned.rcra_cmecomp3
    order by epa_handler_id, receive_date
);
alter table cleaned.event_id add column id int generated always as identity primary key;
